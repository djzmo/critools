...
export async function parseAFS2(buffer) {
...
export async function awb2hcas(awbPath, key?, hcaDir?, type?, skip?) {
...
export async function awb2wavs(awbPath, key?, wavDir?, volume?, mode?, skip?) {
    const pathInfo = path.parse(awbPath);
    console.log(`Parsing ${pathInfo.base}...`);
    const list = await parseAFS2(awbPath);
...
export async function decryptAwb(awbPath, key?, type?) {
    const pathInfo = path.parse(awbPath);
    console.log(`Parsing ${pathInfo.base}...`);
    const list = await parseAFS2(awbPath);
