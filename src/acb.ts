...
import * as afs2 from "./afs2";
import * as hca from "./hca";
...
export async function mixAcb(acbPath, key?, wavDir?, mode?, skip?) {
    const pathInfo = path.parse(acbPath);
...
export async function acb2hcas(acbPath, key?, hcaDir?, type?, skip?) {
    const pathInfo = path.parse(acbPath);
    console.log(`Parsing ${pathInfo.base}...`);
    const acb = await parseAcb(acbPath);
    if (hcaDir === undefined) hcaDir = path.join(pathInfo.dir, acb.Name);
    if (!existsSync(hcaDir)) {
...
export async function acb2wavs(acbPath, key?, wavDir?, volume?, mode?, skip?) {
    const pathInfo = path.parse(acbPath);
    console.log(`Parsing ${pathInfo.base}...`);
    const acb = await parseAcb(acbPath);
    if (wavDir === undefined) wavDir = path.join(pathInfo.dir, acb.Name);
...
export async function decryptAcb(acbPath, key?, type?) {
    const pathInfo = path.parse(acbPath);
    console.log(`Parsing ${pathInfo.base}...`);
    const acb = await parseAcb(acbPath);
